if mods["space-exploration"] and mods["Krastorio2"] then   
    ...
    local function addIngredient(tech, ingredientName, ingredientCount)
		...
		elseif tech.normal.unit.ingredients then 
			for _,ingredient in pairs(tech.normal.unit.ingredients) do
				if ingredient[1] == ingredientName then
					return
				end				
			end
			table.insert(tech.normal.unit.ingredients, {ingredientName, ingredientCount})
		elseif tech.expensive.unit.ingredients then 
			for _,ingredient in pairs(tech.expensive.unit.ingredients) do
				if ingredient[1] == ingredientName then
					return
				end				
			end
			table.insert(tech.expensive.unit.ingredients, {ingredientName, ingredientCount})
		end
    end
